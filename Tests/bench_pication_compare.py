# -*- coding: utf-8 -*-
"""π-阳离子 PerFrame vs TwoPass 性能对比。"""
import time
import tracemalloc
import numpy as np
...
from DuIvyInteractions.interaction_detectors import (
    PiCationDetectorPerFrame, PiCationDetectorTwoPass)
...
rings = [g for g in relevant if g.group_type == "aromatic_ring"]
cations = [g for g in relevant if g.group_type == "charged_positive"]
print(f"Rings: {len(rings)}, Cations: {len(cations)}, "
      f"Candidates: {len(rings) * len(cations)}")
print()
...
print("[PerFrame]")
res_per = bench("PerFrame", PiCationDetectorPerFrame, relevant, u1.trajectory)

# TwoPass 分阶段
u2 = mda.Universe(TPR, XTC)
det_two = PiCationDetectorTwoPass()
...
print(f"\n[TwoPass full]")
res_two_full = bench("TwoPass", PiCationDetectorTwoPass, relevant, u4.trajectory)
...
if it_per and it_two:
    ...
    only_two = two_pairs - per_pairs
    if only_two:
        print(f"\n  TwoPass only pairs:")
        for g1, g2 in it_two.groups:
            if (g1.group_id, g2.group_id) in only_two:
                occ = two_occ[(g1.group_id, g2.group_id)]
                print(f"    {g1.residue_name}{g1.residue_id} -> "
                      f"{g2.residue_name}{g2.residue_id}  occ={occ:.3f}")
